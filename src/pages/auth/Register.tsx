...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { user, error: regError } = await register(email, name, password);
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <div className="text-center mb-8">
            <h1 className="font-serif text-3xl font-bold text-church-earth-dark mb-2">Create Account</h1>
            <p className="text-church-earth-light">Join the Jesus Church community</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-church-earth-dark mb-2">Full Name</label>
              <input
                type="text"
                required
                value={name}
          ...
          <div className="mt-8 text-center text-sm text-church-earth-light">
            ...
            <Link to="/login" className="text-church-gold font-medium hover:underline">
              Sign in
            </Link>
          </div>
        </motion.div>
